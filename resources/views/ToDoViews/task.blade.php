        ...
        <div class="row">
            ...
            <div class="col-10 px-5">
                    ...
                    @foreach($toDoTask as $item)
                        @if($id == $item->id)
                        <form action="{{route("routeUpdateTask", ['id' => $item->id])}}" method="post">
                            @csrf
                            <h1 style="color: #7e30e1">Görev</h1>
                            <input type="text" value="{{$item->taskTitle}}" class="form-control mt-4" placeholder="Görev Başlığı Giriniz" aria-label="Username" aria-describedby="basic-addon1" name="taskTitle" required autocomplete="off">
                            <textarea class="form-control mt-4" rows="5" aria-label="With textarea" placeholder="Görev Giriniz" name="taskContent" required autocomplete="off">{{$item->taskContent}}</textarea>
                            <select class="form-control mt-4" name="taskStatus">
                                <option selected disabled>{{$item->taskStatus}}</option>
                                <option value="Yapılıyor">Yapılıyor</option>
                                <option value="Yapılmadı">Yapılmadı</option>
                                <option value="Ertelendi">Ertelendi</option>
                                <option value="İptal Edildi">İptal Edildi</option>
                            </select>
                            <button type="submit" name="action" value="update" class="btn text-light mt-4" style="background-color: #7e30e1">Kaydet</button>
                            <button type="submit" name="action" value="delete" class="btn btn-danger text-light mt-4 mx-3">Görevi Sil</button>
                        </form>
                        @endif
                    @endforeach
